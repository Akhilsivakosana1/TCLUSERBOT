...
TELEPIC = (
    PMPERMIT_PIC
    if PMPERMIT_PIC
    else "https://telegra.ph/file/a8cf9812c35f9c04eac27.jpg"
)
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
myid = bot.uid
MESAG = (
    str(CUSTOM_PMPERMIT)
    if CUSTOM_PMPERMIT
    else "`TCLBot PM security! Please wait for me to approve you. 😊"
)
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "TCLBot User"
...
@userbot.on(admin_cmd(pattern="a ?(.*)"))
...
@userbot.on(admin_cmd(pattern="block ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
@userbot.on(admin_cmd(pattern="da ?(.*)"))
...
CMD_HELP.update(
    {
        ...
        \nGet help from @Telugucartoonlover."
    }
)
# (c) userBot
